...
//
+ (NSString*) ycu_getYcUdidValue {
    return [YCOpenUDID getYcUdidValueWithError:nil];
}

+ (NSString*) getYcUdidValueWithError:(NSError **)error {
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [YCOpenUDID _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [YCOpenUDID _generateFreshOpenUDID];
        } else {
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [YCOpenUDID _setDict:localDict forPasteboard:slotPB];
    }
...
+ (void) setYcUdidOptOut:(BOOL)optOutValue {
    
    // init call
    [YCOpenUDID ycu_getYcUdidValue];
